...
export async function generateMetadata({
  ...
  const article = await getArticleContent("apps", slug);
  if (!article) return {};
  return {
...
export default async function AppsArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link href="/apps" className="text-teal text-sm hover:text-teal-dark transition-colors mb-6 inline-block">
        &larr; 返回應用新知
      </Link>
      <article>
