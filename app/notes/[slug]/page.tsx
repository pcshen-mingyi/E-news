...
export async function generateMetadata({
  ...
  const article = await getArticleContent("notes", slug);
  if (!article) return {};
  return {
...
export default async function NotesArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link href="/notes" className="text-teal text-sm hover:text-teal-dark transition-colors mb-6 inline-block">
        &larr; 返回 PC 小筆記
      </Link>
      <article>
      ...
      <ReactionBar articleSlug={`notes/${slug}`} />
      <CommentSection articleSlug={`notes/${slug}`} />
    </div>
  );
}
